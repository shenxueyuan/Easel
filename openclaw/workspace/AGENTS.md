...
完成可对外发布的图文或视频内容时，必须在项目目录写 `content.json`，作为对话页「去发布」与发布中心之间的唯一数据源。不要让 Web 页面从自然语言回复中猜标题、正文或媒体。

```json
{
  "content": {
    "title": "对外标题",
    "body": "只包含对外正文，不含执行说明、发布建议或内部路径",
    "tags": ["标签1", "标签2"],
    "media_type": "image",
    "images": ["outputs/项目/card_1.png"]
  },
  "platforms": ["xiaohongshu", "douyin"]
}
```

视频使用 `video` 字段；图集使用 `images`。媒体必须是已经通过自检的真实文件。用户要求修改标题、正文、标签或媒体后，要同步更新同一个 `content.json`。仅做分析、建议或发布状态检查时不得创建或覆盖发布包。
